...
/**
 * Represents the computer opponent and stores a hashmap of 
 * user input patterns. Makes predictions of the user's next moves
 * based on the patterns stored.
...
public class Computer implements Serializable {
	...
	/**
	 * Takes in a pattern of size 4 and checks to see if that pattern
	 * exists. If it does, it adds R S P to the pattern to see if that
	 * pattern exists. If not, it looks for a smaller pattern until it 
	 * finds one. If a pattern is not found, a random move is made.
	...
	public char makePrediction( Pattern pattern ) {
		...
		while (  ( predict.length() - i ) > 1 && !patternFound ) {
			...
			if ( patterns.containsKey( new Pattern( test ) ) ) {
				...
				if ( patterns.containsKey( new Pattern( test ) ) ) {
					r = patterns.get( new Pattern( test ) );
				}
				test = predict.substring( 0, predict.length() - i ) + 'P';
				if ( patterns.containsKey( new Pattern( test ) ) ) {
					p = patterns.get( new Pattern( test ) );
				}
				test = predict.substring( 0, predict.length() - i ) + 'S';
				if ( patterns.containsKey( new Pattern( test ) ) ) {
					s = patterns.get( new Pattern( test ) );
				}
				patternFound = true;
			}
			else {
